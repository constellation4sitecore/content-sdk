# Constellation For Sitecore XM Cloud (Next JS - Sitecore Content SDK)

Constellation for Sitecore XM Clould is an open-source tools for easily develop within Sitecore XMClould + SXA + Next JS (Content SDK)

## SXA Modules

- [Data](src/data/README.md)
- [Enhancers](src/enhancers/README.md)
- [Labels](src/labels/README.md)
- [Layout](src/layout/README.md)
- [Mapper](src/mapper/README.md)
- [Media](src/media/README.md)

- [Navigation](src/navigation/README.md)
- [Url Friendly Page Names](src/url-friendly-page-names/README.md)
- [Security](src/security/README.md)
